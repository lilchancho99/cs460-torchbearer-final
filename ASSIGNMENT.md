...
1. Why a single shortest-path run from S is not sufficient, name the specific decision
   it cannot make.
      So we need the shortest path distances from S to each node and not just S alone.
...
3. In one sentence: why this problem is a search over orders, not a single computation.
      There are k! possible outcomes of relics so no single computation will be able to determine which ordering is cheapest
